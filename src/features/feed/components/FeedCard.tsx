...
const FeedCard = React.forwardRef<HTMLDivElement, FeedCardProps>(
  ({ className, city, country, mood, content, timestamp, isPrivate, index, ...props }, ref) => {
    ...
    return (
      <div className="h-screen w-full snap-start snap-always flex items-center justify-center px-4 relative">
        ...
        <div 
          ...
          style={{ 
            background: `radial-gradient(circle at 50% 50%, var(--mood-${mood.toLowerCase()}), transparent 70%)`
          }}
        />
        ...
        <div className="absolute bottom-8 left-1/2 -translate-x-1/2 flex flex-col items-center gap-2 opacity-60 hover:opacity-100 transition-opacity">
          ...
          <div className="h-6 w-[1px] bg-gradient-to-b from-muted-foreground/60 to-transparent" />
        </div>

        <Card
          ref={ref}
          className={cn(
            "group relative max-w-2xl w-full overflow-hidden rounded-none border-l-4 border-y border-r bg-black/60 backdrop-blur-md transition-all duration-500 hover:bg-black/70 hover:border-l-8 hover:shadow-2xl",
            className
          )}
          style={{ 
            borderLeftColor: `var(--mood-${mood.toLowerCase()})`,
            borderTopColor: `var(--mood-${mood.toLowerCase()})20`,
            borderBottomColor: `var(--mood-${mood.toLowerCase()})20`,
            boxShadow: `0 0 60px -10px var(--mood-${mood.toLowerCase()})40`
          }}
          {...props}
        >
            ...
            <div className="font-mono text-sm uppercase tracking-widest text-muted-foreground space-y-2">
              <div className="flex items-center gap-3">
                <div 
                  ...
                  style={{ backgroundColor: `var(--mood-${mood.toLowerCase()})` }}
                />
                <span className="text-foreground text-base">{city}, {country}</span>
              </div>
              <div className="flex items-center gap-2 pl-5">
                <span className="text-[10px] opacity-50">MOOD_STATE:</span>
                <span 
                  className="font-bold text-xs" 
                  style={{ color: `var(--mood-${mood.toLowerCase()})` }}
                >
                  {mood.toUpperCase()}
                </span>
              </div>
          ...
          {/* Corner Accent */}
          <div 
            className="absolute bottom-0 right-0 h-12 w-12 opacity-20"
            style={{
              background: `linear-gradient(135deg, transparent 50%, var(--mood-${mood.toLowerCase()}) 50%)`
            }}
          />
        </Card>
      </div>
